...
function toVariantRow(v: AdminProductVariant): VariantRow {
  return {
    ...
    images: (v.images ?? [])
      ...
      .map((img) => ({ id: img.id, url: img.url, displayOrder: img.displayOrder, primary: img.primary })),
...
function blankVariantRow(prefill: { sellingPrice: string; costPrice: string }): VariantRow {
  return {
    key: nextLocalKey(),
    sku: '',
    sizeId: '',
...
export default function AdminProductForm() {
  ...
  const applyProduct = useCallback((product: AdminProductDetail) => {
    ...
    setSubCategoryId(product.subCategoryId != null ? String(product.subCategoryId) : '')
    ...
    setBaseSellingPrice(product.baseSellingPrice != null ? String(product.baseSellingPrice) : '')
    setBaseCostPrice(product.baseCostPrice != null ? String(product.baseCostPrice) : '')
  ...
  const addVariant = () => {
    setVariants((prev) => [...prev, blankVariantRow({ sellingPrice: baseSellingPrice, costPrice: baseCostPrice })])
  }
  ...
  const handleImageUploaded = (variantKey: string, url: string | null) => {
    ...
    setVariants((prev) =>
      prev.map((v) => {
        ...
        const newImage: ImageRow = { url, displayOrder: v.images.length, primary: v.images.length === 0 }
  ...
  const setPrimaryImage = (variantKey: string, index: number) => {
    setVariants((prev) =>
      prev.map((v) => {
        if (v.key !== variantKey) return v
        return { ...v, images: v.images.map((img, i) => ({ ...img, primary: i === index })) }
      }),
    )
  }
  ...
  const moveImage = (variantKey: string, index: number, direction: -1 | 1) => {
    setVariants((prev) =>
      prev.map((v) => {
        ...
        return { ...v, images: imgs.map((img, i) => ({ ...img, displayOrder: i })) }
      }),
    )
  }
  ...
  const buildPayload = (): ProductAdminRequest => ({
    ...
    variants: variants.map((v) => ({
      ...
      discountPercent: v.discountPercent.trim() === '' ? 0 : Number(v.discountPercent),
      stockQuantity: v.stockQuantity.trim() === '' ? 0 : Number(v.stockQuantity),
      lowStockThreshold: toNumberOrNull(v.lowStockThreshold),
      active: v.active,
      images: v.images.map((img, idx) => ({ url: img.url, displayOrder: idx, primary: img.primary })),
    })),
  })
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    try {
      ...
      if (isNew) {
        const created = await adminProductsApi.createAdminProduct(payload)
        setSaveMessage('Product created.')
        navigate(`/admin/products/${created.id}`, { replace: true })
      } else {
        const updated = await adminProductsApi.updateAdminProduct(productId!, payload)
  ...
  if (loading) {
    return <div className="flex min-h-[40vh] items-center justify-center text-sm text-zinc-500">Loading product…</div>
  }
  ...
  return (
    <div className="mx-auto max-w-5xl px-4 py-10 sm:px-6 lg:px-8">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-semibold text-zinc-900">{isNew ? 'Add product' : `Edit ${name || 'product'}`}</h1>
          <p className="mt-1 text-sm text-zinc-500">Details, variants, and images for this product.</p>
        </div>
        <Link to="/admin/products" className="text-sm font-medium text-zinc-600 hover:text-zinc-900">
          &larr; All products
        </Link>
      </div>
      ...
      <form onSubmit={handleSubmit}>
        ...
        {activeTab === 'details' && (
          <section className="mt-6 space-y-6">
            <div className="grid grid-cols-1 gap-6 sm:grid-cols-2">
              <TextField label="Name" value={name} onChange={(v) => setName(v ?? '')} />
              <SelectField
                label="Status"
                value={status}
            ...
            <TextField label="Description" value={description} onChange={(v) => setDescription(v ?? '')} textarea />
            ...
            <div className="grid grid-cols-1 gap-6 sm:grid-cols-2">
              ...
              <SelectField
                ...
                placeholder={categoryId ? '— None —' : 'Select a category first'}
                disabled={!categoryId}
            ...
            <div>
              <h3 className="text-sm font-semibold text-zinc-900">Base values</h3>
              <p className="mt-1 text-xs text-zinc-500">
                Used to prefill new variants added below — not required, and not shown to customers directly.
              </p>
              <div className="mt-3 grid grid-cols-1 gap-6 sm:grid-cols-3">
                <TextField label="Base SKU" value={baseSku || null} onChange={(v) => setBaseSku(v ?? '')} />
                <TextField
                ...
                <tbody className="divide-y divide-zinc-100 bg-white">
                  {variants.length === 0 ? (
                    ...
                    variants.map((v, idx) => {
                      ...
                      const colorError = fieldErrors[`variants[${idx}].colorId`]
                      const priceError = fieldErrors[`variants[${idx}].sellingPrice`]
                      return (
                        <tr key={v.key} className="align-top">
                          <td className="px-3 py-2">
                            <select
                              value={v.colorId}
                              onChange={(e) => updateVariant(v.key, { colorId: e.target.value })}
                            ...
                            {colorError && <p className="mt-1 text-[11px] text-rose-600">{colorError}</p>}
                          </td>
                          <td className="px-3 py-2">
                            <select
                              value={v.sizeId}
                              onChange={(e) => updateVariant(v.key, { sizeId: e.target.value })}
                          ...
                          <td className="px-3 py-2">
                            <input
                              ...
                              className="w-32 rounded-md border border-zinc-300 px-2 py-1.5 text-sm"
                            />
                            {skuError && <p className="mt-1 text-[11px] text-rose-600">{skuError}</p>}
                          </td>
                          <td className="px-3 py-2">
                            <input
                              type="number"
                              value={v.sellingPrice}
                              onChange={(e) => updateVariant(v.key, { sellingPrice: e.target.value })}
                              className="w-24 rounded-md border border-zinc-300 px-2 py-1.5 text-sm"
                            />
                            {priceError && <p className="mt-1 text-[11px] text-rose-600">{priceError}</p>}
                          </td>
                          <td className="px-3 py-2">
                            <input
                              type="number"
                          ...
                          <td className="px-3 py-2">
                            {v.id ? (
                              ...
                              >
                                <span className="text-zinc-700">{v.stockQuantity || '0'}</span>
        ...
        {activeTab === 'images' && (
          <section className="mt-6 space-y-8">
            {variants.length === 0 ? (
              <p className="text-sm text-zinc-500">Add at least one variant in the Variants tab before adding images.</p>
            ) : (
              variants.map((v) => (
                <div key={v.key} className="rounded-lg border border-zinc-200 p-4">
                  <h3 className="text-sm font-semibold text-zinc-900">{variantLabel(v, sizes, colors)}</h3>
                  ...
                  {v.images.length > 0 && (
                    <div className="mt-3 flex flex-wrap gap-3">
                      {v.images.map((img, index) => {
                        ...
                        return (
                          <div key={img.id ?? `${img.url}-${index}`} className="w-20 shrink-0">
                            <div className="relative">
                              {url && (
                                <img src={url} alt="" className="h-20 w-20 rounded-md border border-zinc-200 object-cover" />
                              )}
                              {img.primary && (
        ...
        {saveMessage && (
          <div className="mt-8 rounded-md border border-emerald-200 bg-emerald-50 px-3 py-2 text-sm text-emerald-700">
            {saveMessage}
          </div>
        )}
        {saveError && (
